...
export const login = async ({
  ...
  try {
    const loginResult = await request.post('/auth/email/login', {
      email,
      password
    })
...
export const resetPassword = async ({
  ...
  try {
    const resetResult = await request.post('/auth/email/reset/confirm', {
      newPassword,
      confirmedPassword,
      passwordResetId
    })
...
export const createPasswordReset = async ({ email }: { email: string }) => {
  ...
  try {
    const resetResult = await request.post('/auth/email/reset/create', {
      email
    })
    data = resetResult.data
  } catch (err) {
